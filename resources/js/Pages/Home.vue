...
habit.completed.sort((a, b) => new Date(a) - new Date(b)).forEach((date) => {
  if (lastDate) {
...
let completed = ["2025-11-05","2025-11-04","2025-11-03","2025-11-25","2025-11-11"]
...
<template>
  <main class="flex-1 p-8">
    ...
    <form v-if="showForm" @submit.prevent="addHabit" class="max-w-sm mx-auto">
      <Input v-model="name" label="Name for a habit" placeholder="Drinking a water" />
  ...
  <div v-if="habits.length === 1" class="bg-white w-2xl rounded-3xl shadow-xl p-8 mt-8 mb-8 border border-indigo-100">
    <div  class="flex flex-row gap-6 items-center justify-center">

              <div>
                <h2 class="text-3xl font-bold text-gray-800 mb-2">Your Streak Journey</h2>
                <p class="text-gray-500">Keep going! 🚀</p>
              </div>
      
          <div class="bg-[#51A7EE] relative rounded-2xl p-8 text-center text-white ">
            <h1>{{ actualStreak }}⚡︎</h1>
            <p>Actual Streak</p>
          </div>
          <div class="bg-[#F8833D] relative rounded-2xl p-8 text-center text-white">
    ...
    <div class="space-y-6" ref="pdfExportHabit">
      <div v-for="(habit, index) in habits" :key="index" class="habit-card">
        <div class="bg-[#90CAFA] relative rounded-2xl p-8 text-center text-white max-w-2xl mx-auto">
          ...
          <div class="absolute top-4 right-4 flex space-x-3 options">
            ...
            <button @click="deleteHabit(index)" class="text-white hover:text-gray-200 text-xl">
              🗑️
            </button>
          </div>
        </div>
        <div class="h-[10px] my-[30px] relative rounded-2xl p-8 text-center">
          <div>
            <Calendar
              ref="calendar"
              :completed="habit.completed"
              :clicked="habit.clicked"
              @completedChanged="(date, completed) => completedChanged(date, completed, index)"
            />
          </div>
